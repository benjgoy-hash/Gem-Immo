...
class BienIciScraper: 
	...
	def go_api_page(self, params): 
		...
		time.sleep(2)  # délai pour éviter le rate limiting
		response = self.s.get('https://www.bienici.com/realEstateAds.json', params=params, headers=HEADERS)
		if response.status_code == 429:
			print('rate limited (429), attente 30s...')
			time.sleep(30)
			response = self.s.get('https://www.bienici.com/realEstateAds.json', params=params, headers=HEADERS)
		if response.status_code == 403:
			print('accès refusé (403), attente 60s...')
			time.sleep(60)
			response = self.s.get('https://www.bienici.com/realEstateAds.json', params=params, headers=HEADERS)
	...
	def collect_results(self): 
		...
		MAX_API_PAGE = 104  # BienIci refuse les pages > 104 (limite de 2500 résultats par 24)

		while True: 

			assert self.page and isinstance(self.page, int)

			# Arrêt propre avant la limite de l'API
			if self.page > MAX_API_PAGE:
				print('limite API atteinte (page %s), arrêt propre' % MAX_API_PAGE)
				self.write_to_csv()
				return
	...
	def parse_ad(self, ad): 
		...
		# Build the ad URL using 'id' field and 'adTypeFR' (already in French)
		import unicodedata
		ad_id = ad.get("id", "")
		ad_type_fr = ad.get("adTypeFR", "")  # already "vente" or "location"
		DB_TO_FRENCH_SLUG = {
		...
		# Normalize city: remove accents, lowercase, replace spaces/apostrophes with hyphens
		city_normalized = unicodedata.normalize("NFD", city).encode("ascii", "ignore").decode("ascii")
		city_slug = city_normalized.lower().replace(" ", "-").replace("'", "-") if city else ""
		property_type_slug = DB_TO_FRENCH_SLUG.get(property_type, property_type)
		rooms = ad.get("roomsQuantity", "")
		rooms_slug = "%spieces" % rooms if rooms else ""
		url = "https://www.bienici.com/annonce/%s/%s/%s/%s/%s" % (
			ad_type_fr, city_slug, property_type_slug, rooms_slug, ad_id
		)
	...
	def write_to_csv(self): 
		...
		with open(self.output, 'w', encoding='utf-8-sig') as f: 
